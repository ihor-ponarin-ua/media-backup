...
@Component
public class FileNameMonthAndYearBasedStoreStrategy implements StoreStrategy {
    ...
    public Path apply(Path pathToFileOnPortableDevice, Path basePathOnStorageDevice) {
        ...
        if (matcher.find()) {
            ...
            var month = Integer.valueOf(localDate.getMonthValue());
            ...
            throw new IllegalArgumentException(String.format("The filename: '%s' in the path: '%s' doesn't match the pattern", pathToFileOnPortableDevice.getFileName(), pathToFileOnPortableDevice));
        }
    }
